...
import { AlertCircle, TrendingUp, TrendingDown, Activity, Calendar, Info, BookOpen } from 'lucide-react';
...
export function ReportOverview({ onNavigate }: ReportOverviewProps) {
  const healthClarityScore = 68;
  const severityLevel = "Moderate Attention";
  const severityColor = "#C89B3C";

  const abnormalParameters = [
    { name: "Hemoglobin", value: "11.2", unit: "g/dL", status: "below", trend: "down" },
    { name: "Total Cholesterol", value: "245", unit: "mg/dL", status: "above", trend: "up" },
    { name: "Vitamin D", value: "18", unit: "ng/mL", status: "below", trend: "stable" },
  ];

  const normalParameters = 12;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-4">
              ...
              <button className="px-4 py-2 bg-white border border-[#E5E9ED] text-[#2F4A68] rounded-lg hover:bg-[#F7F9FB] transition-colors text-sm flex items-center gap-2">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              ...
              <div className="text-center">
                <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#C89B3C]/10 rounded-full">
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-4 bg-[#2E7D5B]/5 rounded-xl">
                    ...
                    <span className="text-lg text-[#2E7D5B]">{normalParameters}</span>
                  </div>
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-4">
                ...
                <Tooltip.Provider>
                  <Tooltip.Root>
                    <Tooltip.Trigger asChild>
                      <button className="text-[#5E6C7A] hover:text-[#3A9CA6] transition-colors">
                    ...
                    <Tooltip.Portal>
                      <Tooltip.Content
                        ...
                        sideOffset={5}
                      >
                        Plain-language summary to help understand test results without medical jargon
                        <Tooltip.Arrow className="fill-[#2F4A68]" />
                      </Tooltip.Content>
                    </Tooltip.Portal>
                  </Tooltip.Root>
              ...
              <div className="space-y-3 mb-5">
                ...
                <div className="flex items-start gap-3">
                  <div className="w-1.5 h-1.5 bg-[#3A9CA6] rounded-full mt-2 flex-shrink-0"></div>
                  <p className="text-[#1F2933] leading-relaxed">
                    Tracking reports over time helps understand whether health indicators are improving, stable, or changing.
                  </p>
                </div>
              </div>
        ...
        >
          ...
          <div className="space-y-4">
            {abnormalParameters.map((param, index) => (
              <motion.div
                key={param.name}
                ...
                className="p-4 border border-[#E5E9ED] rounded-xl hover:border-[#3A9CA6] hover:shadow-md transition-all cursor-pointer"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                      param.status === 'above' ? 'bg-[#C89B3C]/10' : 'bg-[#7A8088]/10'
                    }`}>
                      {param.trend === 'up' && <TrendingUp className="w-5 h-5 text-[#C89B3C]" />}
                      {param.trend === 'down' && <TrendingDown className="w-5 h-5 text-[#7A8088]" />}
                      {param.trend === 'stable' && <Activity className="w-5 h-5 text-[#5E6C7A]" />}
                    </div>
                    <div>
                      <p className="text-[#1F2933]">{param.name}</p>
                      <p className="text-sm text-[#5E6C7A]">
                        {param.status === 'above' ? 'Above' : 'Below'} reference range
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-lg text-[#1F2933]">{param.value} <span className="text-sm text-[#5E6C7A]">{param.unit}</span></p>
                  </div>
                </div>
              </motion.div>
            ))}
